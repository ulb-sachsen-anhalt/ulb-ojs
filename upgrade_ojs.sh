...
echo  "drop database ojs;" | docker exec -i ojs_db_ulb mysql -p$1

echo "create database ojs"

echo  "create database ojs;" | docker exec -i ojs_db_ulb mysql -p$1

echo "copy public files" | cp -aur /srv/ojs/public/ /data/ojs/
echo "copy journals files" | cp -aur /home/ojs/journals/ /data/ojs/private/
echo "copy usageStats files" | cp -aur /home/ojs/usageStats/ /data/ojs/private/
...
cat /data/ojs/sqldumps/ojs.sql | docker exec -i ojs_db_ulb mysql -p$1 ojs
